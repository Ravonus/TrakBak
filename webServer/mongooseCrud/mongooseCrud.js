            ...
            if (file !== 'mongooseCrud.js' && file.substring(file.length - 3) == ".js") {
              ...
              function finished() {
                ...
                fs.readFile(path.join(__dirname, '../../', 'webServer/routes/', `${myFile.slice(0, -3)}Route.js`), 'utf8', function (err, fileString) {
                  ...
                  config.controllers[capFirst(modelFile).slice(0, -3)].api[myFile.slice(0, -3)] = requireFromString(fileString.replace(/modelName/g, modelFile.slice(0, -3)).replace(/routeType/g, myFile.slice(0, -3)))
